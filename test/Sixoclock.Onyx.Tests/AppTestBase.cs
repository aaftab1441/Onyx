...
using Sixoclock.Onyx.Tests.TestDatas;
...
namespace Sixoclock.Onyx.Tests
{
    ...
    /// Provides methods to easily work with <see cref="OnyxDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<OnyxTestModule>
    {
        ...
        {
            void NormalizeDbContext(OnyxDbContext context)
            {
        ...
        protected void UsingDbContext(Action<OnyxDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<OnyxDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<OnyxDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<OnyxDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<OnyxDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OnyxDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<OnyxDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OnyxDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<OnyxDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<OnyxDbContext, Task<T>> func)
        {
            T result;
