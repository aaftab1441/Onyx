...
{
    [OCPPAction(Name = "ChangeConfiguration", Type = RequestHandlerType.CPTOCS)]
    public class ChangeConfigurationResponseHandler:IRequestHandler
    {
        ...
        public async Task HandleRequestAsync(RPCMessage message)
        {
            ...
            using (var unitofWork = _unitOfWorkManager.Begin())
            {
                ...
                try
                {
                    ...
                    {
                        var messageOb = JsonConvert.DeserializeObject<ChangeConfigurationResponse>(message.PayLoad);
                        var configStatusId = (await _configStatusRepository.FirstOrDefaultAsync(x =>
                                x.Value ==
                                EnumExtensions.ToEnumString<ChangeConfigurationResponseStatusEnum>(messageOb.Status)))
                            .Id;
