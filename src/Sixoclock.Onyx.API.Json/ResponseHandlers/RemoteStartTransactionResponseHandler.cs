...
{
    [OCPPAction(Name = "RemoteStartTransaction", Type = RequestHandlerType.CPTOCS)]
    public class RemoteStartTransactionResponseHandler : IRequestHandler
    {
        ...
        public RemoteStartTransactionResponseHandler(OnyxWebSocketManager webSocketManager, ILogger logger,
            ...
            using (var unitofWork = _unitOfWorkManager.Begin())
            {
                ...
                try
                {
                    ...
                    if (message.MessageId == MessageType.CALLRESULT)
                    {
                        ...
                        var messageOb = JsonConvert.DeserializeObject<RemoteStartTransactionResponse>(message.PayLoad);
                        if (message.Metadata != null && message.Metadata.ContainsKey(APIMetadata.EventId))
                        {
                           
                            int remoteStartEventId = Int32.Parse(message.Metadata[APIMetadata.EventId]);
                            var remoteStartStopStatusId = (await _remoteStartStopStatusRepository.FirstOrDefaultAsync(x =>
                                ...
                                .Id;
                            var remoteStartEvent =
                                await _remoteStartStopEventRepository.FirstOrDefaultAsync(remoteStartEventId);
                            remoteStartEvent.RemoteStartStopStatusId = remoteStartStopStatusId;
                            await _remoteStartStopEventRepository.UpdateAsync(remoteStartEvent);
                        }
