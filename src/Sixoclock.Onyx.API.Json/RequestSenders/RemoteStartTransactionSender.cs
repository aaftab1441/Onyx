...
{
    [OCPPAction(Name = "RemoteStartTransaction", Type = RequestHandlerType.CSTOCP)]
    public class RemoteStartTransactionSender:IRequestHandler
    {
        ...
        public async Task HandleRequestAsync(RPCMessage message)
        {
            using (var unitofWork = _unitOfWorkManager.Begin())
            {
                ...
                try
                {
                    ...
                    {
                        int remoteStartId = Int32.Parse(message.Metadata[APIMetadata.EventId]);
                        var remoteStart = await _remoteStartStopEventRepository.FirstOrDefaultAsync(remoteStartId);
                        remoteStart.OcppMessageEventId = messageEventId;
                        await _remoteStartStopEventRepository.UpdateAsync(remoteStart);
                    }
                }
                catch (Exception ex)
                {
